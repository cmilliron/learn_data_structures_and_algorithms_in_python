class Graph:
    def breadth_first_search(self, v):
        tracking_list  = []
        tracking_queue = []
        tracking_queue.append(v)
        while len(tracking_queue) > 0:
            checking = tracking_queue.pop(0)
            tracking_list.append(checking)
            sorted_neighbors = sorted(self.graph[checking])
            for item in sorted_neighbors:
                if item not in tracking_list and item not in tracking_queue:
