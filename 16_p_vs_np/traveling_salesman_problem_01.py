def tsp(cities, paths, dist):
    # print("in tsp", "Cities Len: ", len(cities))
    all_combos = permutations(cities)
    # print(all_combos)
    for i, path in enumerate(all_combos):
        distance = 0
        for p in range(1, len(path)):
            distance += paths[path[0]][p]
        # For Testing
        # print("Path - ", path, " Resulting distance: ", distance, " Checking Against: ", dist, " Result: ", distance < dist)
        if distance < dist:
            return True
    return False
